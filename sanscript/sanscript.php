...
class Sanscript {
    ...
    private function initializeVariables() {
        ...
        $this->schemes = array(
            ...
            "devanagari" => array(
                ...
                "other" => array("क़", "ख़", "ग़", "ज़", "ड़", "ढ़", "फ़", "य़", "ऱ", "ऴ")
            ),

            /* Gujarati
             * --------
             * Sanskrit-complete.
             * Dandas are changed to PERIOD (.) Characters
             */
            "gujarati" => array(
            ...
            "tamil" => array(
                ...
                "virama" => array("்"),
                "consonants" => explode(' ', 'க க² க³ க⁴ ங ச ச² ஜ ஜ² ஞ ட ட² ட³ ட⁴ ண த த² த³ த⁴ ந ப ப² ப³ ப⁴ ம ய ர ல வ ஶ ஷ ஸ ஹ ள க்ஷ ஜ்ஞ'),
             ...
             */
            "itrans" => array(
                "vowels" => array("a", "A", "i", "I", "u", "U", "RRi", "RRI", "LLi", "LLI", "^e", "e", "ai", "^o", "o", "au"),
                ...
                "other" => array("q", "K", "G", "z", ".D", ".Dh", "f", "Y", "R", "^Z")
            ),
    ...
    public function t($data, $from, $to, $options = NULL) {
        ...
        if ($map['fromRoman']) {
            $data = $this->transliterateRoman($data, $map, $options);
        } else {
            $data = $this->transliterateBrahmic($data, $map, $options);
        }

        // Fix any remaining quotations
        ...
        if ($to === 'malayalam' && $options['enableMalayalamChilluSupport'] === true) {
            ...
            // fix for ര്‍വ. - r^va
            $data = preg_replace('/ര്വ/u', 'ര്‍വ', $data); 
